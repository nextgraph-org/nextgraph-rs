...
Hooks for Svelte (5 and 4), Vue, SolidJS, and React.
...
// `users` is a signal of type `{username: string}[]`
const users = useDeepSignal([{ username: "Bob" }]);
```
...
// `users` behaves like a mutable store and has type `{username: string}[]`
const users = useDeepSignal([{ username: "Bob" }]);
```
