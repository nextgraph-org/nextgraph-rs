...
const config = import.meta.env.NG_DEV3
    ...
    : import.meta.env.NG_DEV
      ...
      : import.meta.env.NG_DEV_LOCAL_BROKER
        ...
        : {
              redirect: `https://${redirect_server}/redir/#/?o=`,
              origin: `https://${redirect_server}`,
          };
...
export const init = async function (
    ...
    if (!window)
        throw new Error(
            "init(callback,..) can only be called from a browser's window"
        );
    if (window.self === window.top) {
        window.location.href =
            config.redirect + encodeURIComponent(window.location.href);
...
function rpc(method: string, args?: any): Promise<any> {
    ...
    if (callback_idx !== undefined) {
        ...
        parent.postMessage({ method, args, port: port2 }, config.origin, [
            port2,
        ]);
