 ...
 * @returns An object with an `insertObject` function and a `close` function
 */
export function createInsertObject<T extends BaseType>(
    shapeType: ShapeType<T>
) {
    const connection = OrmConnection.getConnection(shapeType, []);

    const insertObject = async (object: T) => {
    ...
    return {
        ...
        /** The object needs at least a `@graph` field. */
        insertObject,
    };
}
