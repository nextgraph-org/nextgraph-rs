...
<script lang="ts">
  ...
  function contained(graph) {
    ...
    for (const g of graph) {
      if (g.substring(57, 91) === "http://www.w3.org/2006/vcard/ns#fn") {
        ...
        g.substring(57, 97) === "http://www.w3.org/2006/vcard/ns#hasEmail"
      ) {
  ...
  async function test() {
    await on_qr_scanned(
      "AgBWW2hNMS_1xjz5Pvscq1QQ5D405EWRjoN70kIuAgNIqwABALRXYVWKYVaz6xqNqncU7D2HyheZKlFepLoJVE-xIbywB25pa28gMTMBD2ZpcmVmb3hAcHJvLmNvbQ"
    );
  }
</script>
...
<div class="flex-col p-5" bind:this={container}>
  ...
  {#if !has_camera && !has_name}
    <Alert class="m-2" color="red" style="word-break: break-word;"
      >No camera available. You cannot import with QR-code</Alert
    >
  {/if}
