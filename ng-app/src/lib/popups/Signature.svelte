...
<script lang="ts">
  ...
  import { cur_tab, show_doc_popup } from "../../tab";
  import { get } from "svelte/store";
  import { onMount, onDestroy, tick } from "svelte";
  import ng from "../../api";
  import { t } from "svelte-i18n";
  import { ShieldExclamation, ShieldCheck, Camera } from "svelte-heros-v2";
  import { Toggle, Button } from "flowbite-svelte";
  let is_tauri = import.meta.env.TAURI_ENV_PLATFORM;

  let heads = [];
  onMount(async () => {
    heads = await ng.signature_status(
  ...
  async function sign() {
    if (snapshot) await sign_snapshot();
      ...
      try {
        let immediate = await ng.signature_request(
          $active_session.session_id,
          "did:ng:" + $cur_tab.branch.nuri + ":" + $cur_tab.store.overlay
        );
        if (immediate) {
  ...
  async function sign_snapshot() {
    try {
      let immediate = await ng.signed_snapshot_request(
        $active_session.session_id,
        "did:ng:" + $cur_tab.branch.nuri + ":" + $cur_tab.store.overlay
      );
      if (immediate) {
...
<div class="flex flex-col">
  ...
  {#each heads as head}
    {#if head[1]}
      <div
        style="font-family: monospace; font: Courier; font-size:16px;"
        class="flex text-green-600 clickable my-2"
        on:click={() => (cur_link = signed_commit_link(head))}
        on:keypress={() => (cur_link = signed_commit_link(head))}
        tabindex="0"
        role="button"
      >
  ...
  {#if !hide_snapshot}
    ...
    {:else if can_sign}
      ...
      {#if has_signatures}<span>{$t("doc.signature.or_click_on_head")}</span
        >{/if}
    {:else}
      <div class="flex mt-3">
        <Camera tabindex="-1" class="w-6 h-6 mr-3 text-green-600" /><span
          class="text-green-600">{$t("doc.signature.signed_snap_at_head")}</span
        >
      </div>
      <span
        >{$t("doc.signature.here_is_the_link")}<br />{$t(
          "doc.signature.cli_warning"
        )} :<br /><br /></span
      >
      <span
        style="font-family: monospace; font: Courier; font-size:16px;"
        class="break-all">ngcli get {signed_commit_link(heads[0])}</span
      >
    {/if}
  {/if}
  {#if (force_snapshot || can_sign) && cur_link}
    ...
    <span
      style="font-family: monospace; font: Courier; font-size:16px;"
      class="break-all">ngcli get {cur_link}</span
    >
  {/if}
</div>
