    ...
    async apply(target, path, caller, args) {
        try {
            if (path[0] === "open_window") {
                let callback = args[3];
                let already_exists = await invoke(path[0], {
                    url: args[0],
                    label: args[1],
                    title: args[2]
                });
                ...
                unsub_register_accepted = await listen("accepted", async (event) => {
                    unsub_register();
                ...
                switch (tauri_platform) {
                    case "macos":
                    case "darwin":
                        client_type = "NativeMacOS";
                        break;
                    case "linux":
                        client_type = "NativeLinux";
                        break;
                    case "windows":
                        client_type = "NativeWin";
                        break;
                    case "android":
                        client_type = "NativeAndroid";
                        break;
                    case "ios":
                        client_type = "NativeIos";
                        break;
                }
                ...
                let request;
                let callback;
                if (path[0] === "app_request_stream") {
                    request = args[0];
                    callback = args[1];
                } else if (path[0] === "doc_subscribe") {
                    ...
                    callback = args[4];
                } else if (path[0] === "orm_start_discrete") {
                    request = await invoke("new_orm_start_discrete", {
                        nuri: args[0],
                        session_id: args[1]
                    });
                    callback = args[2];
                } else if (path[0] === "file_get") {
                    request = await invoke("new_file_get", {
                        nuri: args[1],
                        branch_nuri: args[2],
                        session_id: args[0]
                    });
                ...
                let unlisten = await getCurrentWindow().listen(stream_id, async (event) => {
                    ...
                    if (event.payload.V0) {
                        if (event.payload.V0.FileBinary) {
                            event.payload.V0.FileBinary = Uint8Array.from(
                                event.payload.V0.FileBinary
                            );
                        }
                        ...
                        // }
                        if (event.payload.V0.State?.graph?.triples) {
                            event.payload.V0.State.graph.triples = Uint8Array.from(
                                event.payload.V0.State.graph.triples
                            );
                        } else if (event.payload.V0.Patch?.graph) {
                            event.payload.V0.Patch.graph.inserts = Uint8Array.from(
                                event.payload.V0.Patch.graph.inserts
                            );
                            event.payload.V0.Patch.graph.removes = Uint8Array.from(
                                event.payload.V0.Patch.graph.removes
                            );
                        }
                        if (event.payload.V0.State?.discrete) {
                            let crdt = Object.getOwnPropertyNames(
                                event.payload.V0.State.discrete
                            )[0];
                            event.payload.V0.State.discrete[crdt] = Uint8Array.from(
                                event.payload.V0.State.discrete[crdt]
                            );
                        } else if (event.payload.V0.Patch?.discrete) {
                            let crdt = Object.getOwnPropertyNames(
                                event.payload.V0.Patch.discrete
                            )[0];
                            event.payload.V0.Patch.discrete[crdt] = Uint8Array.from(
                                event.payload.V0.Patch.discrete[crdt]
                            );
                        }
                    }
                ...
                if (res)
                    for (let e of Object.entries(res)) {
                        ...
                        if (sec) e[1].wallet.V0.content.security_img = Uint8Array.from(sec);
                    }
                return res || {};
                ...
                encrypted_wallet.V0.content.security_img = Array.from(
                    new Uint8Array(encrypted_wallet.V0.content.security_img)
                );
                return await invoke(path[0], {
                    encrypted_wallet,
                    opened_wallet: args[1],
                    in_memory: args[2]
                });
                ...
                path[0] === "wallet_open_with_pazzle" ||
                path[0] === "wallet_open_with_mnemonic_words" ||
